...
<p align="center">
  一头牛替你看全局；Agent 多起来，就叫来一群。<br>
  每个 Session 一头，只盯自己认领的活。谁停下来等你，牛先开口。
</p>
...
一头牛能告诉你“现在谁在等”。Session 多起来以后，问题会再往前一步：**这头牛到底在替谁操心？**

打开牛群，每个正在拉犁、停犁或吃草的 Session 都会随机认领一头牛。同一次运行里，牛和 Session 的关系不会乱；新牛出现时会先把责任说清楚：

> 这摊我盯着：Codex · niul.ai

牛记和大盘也不再挤在同一头牛身上。牛记牛只替你惦记便签和提醒；开启大盘后，大盘牛只守行情。一个 Session 状态变了，不会惊动整群。

点 Session 牛时，牛来不会立刻把你从当前窗口拽走。它会先展开气泡，显示全部 Session，滚到并高亮那头牛认领的一行；确认无误后，再点那一行才切换 Runtime。拖任意一头牛会移动整群，双击只摸当前这头，Roll 只重抽 Session 牛的造型，不会换掉它们的工作。

牛群默认关闭，在“外观与声音”里开启。关闭后，原来的牛、马或牛马组合会原样回来。

<p align="center">
  <img src="assets/niulai-herd-mode.jpg" width="72%" alt="牛群模式为每个 AI Session 分配一头专属牛，并保留牛记牛和大盘牛">
  <br>
  <sub>画面使用 Mock Session；牛群布局、职责气泡和角色资产均来自实际应用。</sub>
</p>
...
今日 Token 下面现在也能看 Claude 与 Codex 的订阅窗口。点开后会分别列出 5 小时、7 天，以及 Claude 返回的 Fable 等独立模型额度，并显示剩余比例和重置时间。

这项能力默认关闭。需要时到“设置 → 额度”开启，并可分别关闭 Claude 或 Codex。牛来只在主进程内存中读取独立 Claude Code / Codex CLI 的本机登录，只把请求发给对应的 Claude / OpenAI 域名；不会保存、打印或传给其他服务，也不会替你刷新或改写登录凭据。Claude Desktop 的登录不会共享给其他应用；只使用 Desktop 的用户需先在终端运行一次 `claude auth login`。

额度接口不是公开稳定 API，未来可能因上游变化暂时失效。失败时牛来会保留上次数据并自动重试，Session 巡视和本机 Token 统计不受影响。
...
<p align="center">
  ...
  <br>
  <sub>行情画面为 Mock 演示数据。实际行情来自腾讯免 Key 接口，并以东方财富补充缺失数据，可能存在延迟。</sub>
</p>
...
一头牛盯 Session 有点孤单，于是马来了。

马来不是给牛换一张皮：它有自己的暖栗棕短绒、长脸、乱鬃毛、半眯眼和一声不太标准的「咴」。它会眨眼、抬头看你正在 Hover 的 Session，也会跟着台词认真张嘴——虽然看起来依然不太聪明。

- **牛模式**：只有牛来，五连点进入「哞拉松」
- **马模式**：只有朝右看的马来，五连点真的开始「马拉松」
- **牛马模式**：马站左边、牛站右边，两个一起盯、一起张嘴、一起出声

三种模式共享同一套 Session 状态、Token、大盘、Memo、拖动、抚摸和提醒能力；在「外观与声音」里切换并保存即可。

<p align="center">
  <img src="assets/niulai-pet-modes.png" width="88%" alt="牛来设置中切换牛模式、马模式和马左牛右的牛马模式">
  <br>
  <sub>Mock 演示画面。马来与牛来均使用应用内实际角色资产。</sub>
</p>

## ⚠️ 一些没必要，但不能没有的东西

- **单击单宠**：展开或收起名单
- **单击 Session 牛**：展开名单并高亮它认领的 Session；再点列表行才切换 Runtime
- **单击牛记牛 / 大盘牛**：打开各自的面板
- **双击桌宠**：摸一下；牛群里只摸当前这头
- **拖动桌宠**：把它放到不挡工作的地方；牛群里拖任意一头都会移动整群
- **右键单宠**：打开“牛记”，快速记一条便签，可选 15 分钟、1 小时或明早提醒
- **`⌘⇧U`**：随时召唤或隐藏
- **状态变化**：文字和嘴型会按模式使用「哞」「咴」或「哞咴」；抚摸与五连点彩蛋才会播放角色叫声

> [!CAUTION]
> **不要连续点击桌宠五下。**
>
> 牛模式会进入持续五分钟的「哞拉松」，马模式是会马叫的「马拉松」，牛马模式则两个一起跑。想停就再连点五下——前提是你还点得到它们。
...
- 分别调整桌宠和 Session 气泡的大小
- 切换牛模式、马模式或马左牛右的牛马模式
- 开启牛群，让每个实时 Session、牛记和已启用的大盘各有一头只盯自己的牛
- 开关状态播报和碎嘴
- 选择要巡视哪些 AI Runtime
