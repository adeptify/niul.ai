...
- 用户可在“外观与声音”开启默认关闭的牛群模式；它暂时接管角色区，为每个工作中、等你或闲置 Session 映射一头本次运行内稳定的随机牛，并保留牛记牛及已启用的大盘牛。关闭后恢复此前的牛、马或牛马组合与造型选择。
- 牛群中每头 Session 牛只接收绑定 Session 的状态和等待提醒；牛记牛只接收 Memo 保存与到期提醒，大盘牛只接收行情涨跌与失败状态。Session 牛点击只定位并高亮列表行，用户再点列表行才切换 Runtime。
- 牛群共用拖动、Roll 和五连点哞拉松；双击只抚摸当前牛。批量出现时每头牛展示职责短句，但读屏只播报一条群体摘要，声音只播放一层群体哞声。
